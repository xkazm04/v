...
const VideoCardWrapper: React.FC<VideoCardWrapperProps> = ({
  ...
  const { cardColors, colors, isDark, subtone } = useLayoutTheme();
  ...
  return (
    <motion.article
      ...
      onClick={onClick}
    >
      {/* ✅ **Enhanced background effects with subtone support** */}
      <div className="absolute inset-0">
        ...
        {/* Dynamic hover gradient with subtone */}
        <motion.div
          className="absolute inset-0 rounded-xl"
          style={{
            background: subtone.isActive 
              ? `linear-gradient(135deg, ${subtone.color}10, ${subtone.color}05)`
        ...
        <motion.div
          ...
          style={{
            background: `radial-gradient(circle at 50% 50%, ${subtone.isActive ? subtone.color : colors.primary}15 0%, transparent 60%)`
          }}
        />
      </div>
      ...
      {/* ✅ **Enhanced sweep animation with subtone** */}
      {contentLoaded && (
        <motion.div
          className="absolute inset-0 pointer-events-none"
          style={{
            background: `linear-gradient(45deg, transparent 30%, ${subtone.isActive ? subtone.color : colors.primary}08 50%, transparent 70%)`
          }}
          animate={{
            x: isHovered ? '100%' : '-100%',
      ...
      >
        <div 
          ...
          style={{ backgroundColor: subtone.isActive ? subtone.color : colors.primary }}
        />
      </motion.div>
    </motion.article>
  );
