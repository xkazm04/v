...
const PredefinedTweets: React.FC<PredefinedTweetsProps> = ({
  ...
  const getCategoryColors = (category: string, index: number) => {
    const categoryColorMap: Record<string, { bg: string; text: string }> = {
      'Climate Science': isDark 
        ? { bg: 'rgba(34, 197, 94, 0.2)', text: 'rgba(34, 197, 94, 0.9)' }
        : { bg: 'rgba(34, 197, 94, 0.1)', text: 'rgba(34, 197, 94, 0.8)' },
      'Public Health': isDark
        ? { bg: 'rgba(239, 68, 68, 0.2)', text: 'rgba(239, 68, 68, 0.9)' }
        : { bg: 'rgba(239, 68, 68, 0.1)', text: 'rgba(239, 68, 68, 0.8)' },
      'Technology': isDark
        ? { bg: 'rgba(168, 85, 247, 0.2)', text: 'rgba(168, 85, 247, 0.9)' }
        : { bg: 'rgba(168, 85, 247, 0.1)', text: 'rgba(168, 85, 247, 0.8)' },
      'Economics': isDark
        ? { bg: 'rgba(251, 191, 36, 0.2)', text: 'rgba(251, 191, 36, 0.9)' }
        : { bg: 'rgba(251, 191, 36, 0.1)', text: 'rgba(251, 191, 36, 0.8)' },
      'Space Science': isDark
        ? { bg: 'rgba(59, 130, 246, 0.2)', text: 'rgba(59, 130, 246, 0.9)' }
        : { bg: 'rgba(59, 130, 246, 0.1)', text: 'rgba(59, 130, 246, 0.8)' }
    };

    // Fallback to index-based coloring if category not found
    return categoryColorMap[category] || (index % 2 === 1 
      ? { 
          bg: isDark ? 'rgba(236, 72, 153, 0.2)' : 'rgba(236, 72, 153, 0.1)',
          text: isDark ? 'rgba(236, 72, 153, 0.9)' : 'rgba(236, 72, 153, 0.8)'
        }
      : { 
          bg: isDark ? 'rgba(59, 130, 246, 0.2)' : 'rgba(59, 130, 246, 0.1)',
          text: isDark ? 'rgba(59, 130, 246, 0.9)' : 'rgba(59, 130, 246, 0.8)'
        }
    );
  };
      ...
      >
        <div className="space-y-4 relative">
          {predefinedTweets.map((tweet, index) => {
            const categoryColors = getCategoryColors(tweet.category, index);
